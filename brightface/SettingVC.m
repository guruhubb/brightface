...
- (void)viewDidLoad
{
    ...
    editArr = [[NSArray alloc]initWithObjects:
               @"photo format for frame",@"photo background color",@"auto-filter",@"auto-save to camera roll",@"add watermark",@"output pixel format",@"enable pan",
...
//[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://itunes.apple.com/app/850204569"]];
    ...
    [storeProductViewController loadProductWithParameters:@{SKStoreProductParameterITunesItemIdentifier : @"952786359"} completionBlock:^(BOOL result, NSError *error) {
        if (error) {
...
//     [Flurry logEvent:@"Settings - Customer Feedback"];
    ...
    [pickerMail setToRecipients:[NSArray arrayWithObject:@"brightfaceapp@gmail.com"]];
    // Fill out the email body text
    NSString *emailBody = @"hi, I have the following feedback on brightface...";
...
//        }
        if (indexPath.section == 1) {
            ...
            if(indexPath.row==2){
                [cell.textLabel setText:[editArr objectAtIndex:12]];
                [cell setAccessoryType:UITableViewCellAccessoryDisclosureIndicator];
            }
        }
...
//    if (indexPath.section == 1) {
//        if (indexPath.row==0) {
//            NSURL *instagramURL = [NSURL URLWithString:@"instagram://user?username=brightface"];
...
//        }
//        if (indexPath.row==2) {
//            NSURL *twitterURL = [NSURL URLWithString:@"twitter://user?screen_name=brightface"];
...
{
    UIActionSheet *popupQuery;
    popupQuery = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"cancel" destructiveButtonTitle:nil otherButtonTitles:@"enable pan",@"disable pan",nil];
    popupQuery.tag=5;
    [popupQuery showInView:self.view];
}
-(void)pixelAction
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
        if (actionSheet.tag == 0) {
            ...
            if (buttonIndex==1){
                [defaults setBool:NO forKey:@"pan"];
            }
            else if (buttonIndex==0)[defaults setBool:YES forKey:@"pan"];
        }
